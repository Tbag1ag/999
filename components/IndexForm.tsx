...
import { X, Thermometer, Send, Percent } from 'lucide-react';
...
const IndexForm: React.FC<IndexFormProps> = ({ initialData, onSave, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center p-4 bg-[#12141c]/60 backdrop-blur-3xl animate-in fade-in duration-500">
      <div className="bg-white dark:bg-[#1a1d26] rounded-[3rem] shadow-2xl w-full max-w-md overflow-hidden border border-white/10 animate-in slide-in-from-bottom-20 sm:zoom-in-95 duration-500">
        <div className="px-8 py-6 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-white/2">
          ...
          <button type="button" onClick={onCancel} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><X className="w-5 h-5 text-gray-400" /></button>
        </div>
