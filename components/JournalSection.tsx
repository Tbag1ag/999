  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-0">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-12">
        <div className="flex stadium-nav p-1.5 gap-1 shadow-inner">
          {['全部', '随笔', '新闻', '逻辑'].map((t) => (
            <button
              key={t}
      ...
      {/* Intelligence List */}
      <div className="relative pl-6 sm:pl-10 border-l border-white/10 space-y-8 pb-20">
            ...
            return (
              <div key={entry.id} className="relative">
                ...
                <div className={`absolute -left-[45px] sm:-left-[57px] top-4 w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-500 z-10 ${isExpanded ? 'bg-market-dark dark:bg-amber-500 text-white scale-110 shadow-xl' : 'bg-gray-800/80 text-gray-400 border border-white/5'}`}>
                   {React.cloneElement(style.icon as React.ReactElement, { className: 'w-4.5 h-4.5' })}
                </div>

                {/* Collapsible Card */}
                <div 
                  className={`bg-glass rounded-[2.5rem] overflow-hidden transition-all duration-700 ${isExpanded ? 'ring-2 ring-amber-500/20 shadow-2xl scale-[1.01]' : 'hover:bg-white/15 cursor-pointer'}`}
                  onClick={() => !isExpanded && toggleExpand(entry.id)}
                >
                  <div className="p-7 sm:p-9 flex flex-col items-start gap-4">
                    <div className="flex items-center gap-4">
                       <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${style.bg} ${style.color}`}>
                         {entry.type}
                       </span>
                       <div className="flex items-center gap-2 text-[11px] font-black text-[#12141c] dark:text-white uppercase tracking-widest opacity-80">
                    ...
                    <div className="flex items-center justify-between w-full gap-6">
                      <h3 className={`text-2xl sm:text-3xl font-black tracking-tighter leading-tight text-[#12141c] dark:text-white`}>
                        {entry.title || "未命名简报"}
                      </h3>
                      <button 
                        onClick={(e) => { e.stopPropagation(); toggleExpand(entry.id); }}
                        className={`p-2 rounded-full hover:bg-black/5 transition-all duration-500 ${isExpanded ? 'rotate-180' : ''}`}
                      >
                         <ChevronDown className={`w-7 h-7 ${isExpanded ? 'text-market-dark dark:text-amber-500' : 'text-gray-400'}`} />
                      </button>
                    </div>
                  </div>
                  ...
                  >
                    <div className="px-9 pb-12 pt-2 border-t border-black/5 dark:border-white/5">
                      <div className="prose prose-invert max-w-none">
                        <p className="text-[17px] sm:text-[19px] text-black dark:text-white leading-relaxed font-black whitespace-pre-wrap tracking-tight">
                          {entry.content}
                        </p>
                      </div>
                         ...
                         {isAdmin && (
                           <div className="flex gap-2.5">
                             <button 
                               ...
                               className="px-6 py-2.5 bg-black/5 dark:bg-white/5 text-[11px] font-black text-gray-600 dark:text-gray-300 hover:text-amber-500 transition-all rounded-2xl"
                             >
                               EDIT
                             </button>
                             <button 
                               onClick={(e) => { e.stopPropagation(); onDelete(entry.id); }} 
                               className="px-6 py-2.5 bg-red-500/10 text-[11px] font-black text-red-500 hover:bg-red-500 hover:text-white transition-all rounded-2xl"
                             >
                               DELETE
                             </button>
                           </div>
