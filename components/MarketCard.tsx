...
const MarketCard: React.FC<MarketCardProps> = ({ insight, onEdit, onDelete, isEditable = false }) => {
  ...
  return (
    <div 
      ...
      className={`group relative bg-glass rounded-[3rem] p-8 sm:p-10 flex flex-col min-h-[460px] transition-all duration-500 ${isEditable ? 'cursor-pointer hover:ring-2 hover:ring-amber-500/30 hover:-translate-y-2' : 'hover:-translate-y-1.5'}`}
    >
      {/* Admin Mode Badge */}
      {isEditable && (
      ...
      <div className="flex-grow space-y-8">
        ...
        <div>
          <div className="strategy-box p-6 rounded-[1.5rem] bg-white/20 dark:bg-black/10">
      ...
      <div className="mt-8 pt-6 border-t border-black/5 dark:border-white/5 flex items-center justify-between">
         <div className="flex items-center gap-2.5">
            <span className="px-3.5 py-1.5 bg-black/5 dark:bg-white/10 rounded-lg text-[10px] font-black text-gray-700 dark:text-gray-300 uppercase tracking-widest">
              {insight.category}
            </span>
            <div className="flex items-center gap-1 text-[10px] font-black text-[#12141c] dark:text-white uppercase tracking-tighter">
         ...
         {isEditable && (
           <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
              <button 
                ...
                className="p-2.5 bg-white/10 hover:bg-amber-500 hover:text-white rounded-xl text-gray-500 transition-all"
              >
                <Edit2 className="w-3.5 h-3.5" />
              </button>
              <button 
                onClick={(e) => { e.stopPropagation(); onDelete(insight.id); }} 
                className="p-2.5 bg-white/10 hover:bg-red-500 hover:text-white rounded-xl text-gray-500 transition-all"
              >
